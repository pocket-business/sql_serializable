...
extension PostgresType on SqlType {
  ...
  String? get referencedTable => switch (this) {
    SimpleType() || EnumType() || ListType() || SetType() || MapType() => null,
  ...
  Table? getDependants(Table generated) => switch (this) {
    ...
    ListType() || SetType() || MapType() => (this as KeyedCollectionType).storageTableFor(generated),
  };

  Future<dynamic> decode(dynamic sql, PostgresDatabase database) async => switch (this) {
    SimpleType() => switch (this as SimpleType) {
    ...
    EnumType() => sql,
    ReferenceType(table: final referencedTable) => await database.get(referencedTable, sql as int),
    ListType() || SetType() || MapType() =>
      throw UnsupportedError('Cannot decode ListType, SetType or MapType'),
  };

  Future<dynamic> encode(dynamic dart, PostgresDatabase database) async => switch (this) {
     SimpleType() => switch (this as SimpleType) {
  ...
  Future<dynamic> fetch(int id, Table table, PostgresDatabase database) async => switch (this) {
    SimpleType() || EnumType() || ReferenceType() =>
      throw UnsupportedError('Cannot fetch SimpleType, EnumType or ReferenceType'),
    ListType() || SetType() || MapType() => () async {
      final storageTable = (this as KeyedCollectionType).storageTableFor(table);

      final valueColumn = storageTable.columns.singleWhere((column) => column.name == 'value');
      final valueType = (this as KeyedCollectionType).valueType;

      final keyColumn = storageTable.columns.singleWhere((column) => column.name == 'key');
      final keyType = (this as KeyedCollectionType).keyType;

      final valueColumnName = valueType.hasColumn ? valueColumn.name : valueColumn.nullMarkerName;
      final keyColumnName = keyType.hasColumn ? keyColumn.name : keyColumn.nullMarkerName;

      final result = (await database.context.query(
      ...
      final mappedResult = Map.fromEntries(await Future.wait(result.map((row) async {
        final dynamic key;
        final dynamic value;
        ...
        if (valueType.hasColumn) {
          ...
          value = await valueType.fetch(row['id'], storageTable, database);
        }
      ...
      if (this is MapType) {
        ...
        return List.generate(mappedResult.length, (index) => mappedResult[index]);
      }
    }(),
  };
  ...
  Future<void> put(
    ...
    SimpleType() || EnumType() || ReferenceType() =>
      throw UnsupportedError('Cannot put SimpleType, EnumType or ReferenceType'),
    ListType() || SetType() || MapType() => () async {
      ...
      if (value is List) {
        value = Map.fromEntries((value as List).mapIndexed((index, element) => MapEntry(index, element)));
      } else if (value is Set) {
        value = Map.fromEntries((value as Set).mapIndexed((index, element) => MapEntry(index, element)));
      } else {
        value = value as Map;
      }
      ...
      for (final entry in (value as Map).entries) {
        ...
        if (valueType.hasColumn) {
          if (entry.value == null) {
            ...
            substitutions[valueName] = await valueType.encode(entry.value, database);
          }
        } else {
        ...
        if (keyType.hasColumn) {
          if (entry.key == null) {
            ...
            substitutions[keyName] = await keyType.encode(entry.key, database);
          }
        } else {
      ...
      for (final rowAndEntry in IterableZip(<Iterable<dynamic>>[result, (value as Map).entries])) {
        final row = rowAndEntry[0] as PostgreSQLResultRow;
